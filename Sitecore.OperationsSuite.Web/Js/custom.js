...
function showDeleteDialog(sender) {
    // set the id of the record that we want to delete
    jQuery('#deleteRecordId').val(sender.target.id);
...
function deleteData(sender, deleteUrl) {

    var recordToBeDeleted = jQuery('#deleteRecordId').val();

    jQuery.ajax({
        url: deleteUrl,
        data: JSON.stringify({ id: recordToBeDeleted }),
        type: "POST",
        contentType: "application/json;charset=utf-8",
        success: function (data) {
            jQuery('#modal').modal('hide');

            // Remove row that was deleted
            jQuery('#' + recordToBeDeleted).fadeOut(500);
...
jQuery(document).ready(function () {
    ...
    // Setup Spinner
    //jQuery(".input-spinner").spinner({ min: 1, max: 100, increment: 2 });
